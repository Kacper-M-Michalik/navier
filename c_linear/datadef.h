...
#define LOC(x, y) ((x) * (jmax+2) + (y))

/* Macros for poisson(), denoting whether there is an obstacle cell
 * adjacent to some direction
 */
#define eps_E ((flag[LOC(i+1, j)] & C_F)?1:0)
#define eps_W ((flag[LOC(i-1, j)] & C_F)?1:0)
#define eps_N ((flag[LOC(i, j+1)] & C_F)?1:0)
#define eps_S ((flag[LOC(i, j-1)] & C_F)?1:0)
