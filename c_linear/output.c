...
// 	      pmax = max(pmax, p[LOC(i, j)]);
// 	      pmin = min(pmin, p[LOC(i, j)]);
// 	      //umax = max(umax, u[i][j]);
// 	      //umin = min(umin, u[i][j]);
// 	      //vmax = max(vmax, v[i][j]);
// 	      //vmin = min(vmin, v[i][j]);
...
// 		if (outmode == 0) {
//                     double z = (i < imax && j < jmax)?zeta[LOC(i, j)]:0.0;
//                     r = g = b = pow(fabs(z/12.6),.4) * 255;
//                 } else if (outmode == 1) {
//                     double p = (i < imax && j < jmax)?psi[LOC(i, j)]:0.0;
//                     r = g = b = (p+3.0)/7.5 * 255; 
//                 } else if (outmode == 2) {
// 		  r = g = b = (p[LOC(i, j)]-pmin)/(pmax-pmin) * 255;
// 		}
...
//                 (flag[LOC(i, j+1)] & C_F) && (flag[LOC(i+1, j+1)] & C_F)) {
//                 zeta[LOC(i, j)] = (u[LOC(i, j+1)]-u[LOC(i, j)])/dely
//                             -(v[LOC(i+1, j)]-v[LOC(i, j)])/delx;
//             } else {
//                 zeta[LOC(i, j)] = 0.0;
//             }
//         }
